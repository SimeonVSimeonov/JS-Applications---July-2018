function startApp() {
    ...
    function navigateTo(eve) {
        $('section').hide();
        let target = $(eve.target).attr('data-target');
        $('#' + target).show();
    }
    ...
    let requester = (() => {
        ...
        const appKey = 'kid_ByIX2zsEX';
        const appSecret = '8043e7b54e5740bf95bfc177cceff447';
    ...
    async function loadAds() {
        ...
        for (let ad of data) {
            ...
            html.append(`<div>Price: ${Number(ad.price).toFixed(2)} | Publisher: ${ad.publisher}</div>`);
            adsDiv.append(html);
